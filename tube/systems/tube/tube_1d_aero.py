...
class Tube1DAero(System):
    ...
    def setup(self):
        ...
        # geometry
        self.add_inward('area_throat', 1., unit='', desc='area throat relative to area inlet')

        # aero
        self.add_inward('Ps_out', 50000., unit='pa', desc='exit static pressure')
    ...
    def q_from_wpt(self, w, pt, tt, area, subsonic):
        ...
        if isinstance(w, type(np.zeros((1)))):
            mach = np.array([gas.mach(pt[i], tt[i], ru[i], subsonic=subsonic) 
